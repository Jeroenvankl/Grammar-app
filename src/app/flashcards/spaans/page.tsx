...
import { spanishTravelPhrases, SpanishTravelPhrase } from "@/data/phrases/spanish-travel";

function getSpanishVoice(): SpeechSynthesisVoice | null {
  const voices = speechSynthesis.getVoices();
  const esLatam = voices.filter(
    (v) => v.lang === "es-MX" || v.lang === "es-AR" || v.lang === "es-CO" || v.lang === "es-419" || v.lang === "es-US"
  );
  const esAll = voices.filter((v) => v.lang.startsWith("es"));
  const preferred = ["Google español", "Paulina", "Microsoft Dalia", "Sabina", "Angelica", "Jimena", "Monica"];
  for (const name of preferred) {
    const match = esLatam.find((v) => v.name.includes(name));
    if (match) return match;
  }
  for (const name of preferred) {
    const match = esAll.find((v) => v.name.includes(name));
    if (match) return match;
  }
  return esLatam[0] || esAll[0] || null;
}
...
export default function FlashcardsSpaansPage() {
  const allPhrases = useMemo(
    () => spanishTravelPhrases.flatMap((cat) => cat.phrases),
    []
  );
  const [cards, setCards] = useState<SpanishTravelPhrase[]>([]);
  ...
  useEffect(() => {
    const phrases = selectedCategory === "all"
      ? allPhrases
      : spanishTravelPhrases.find((c) => c.id === selectedCategory)?.phrases || allPhrases;
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold text-stone-800">Flashcards Spaans</h1>
          </div>
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
              selectedCategory === "all" ? "bg-amber-600 text-white" : "bg-stone-100 text-stone-600 hover:bg-stone-200"
            }`}
          >
            Alle ({allPhrases.length})
          </button>
          {spanishTravelPhrases.map((cat) => (
            <button
              key={cat.id}
              onClick={() => setSelectedCategory(cat.id)}
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                selectedCategory === cat.id ? "bg-amber-600 text-white" : "bg-stone-100 text-stone-600 hover:bg-stone-200"
              }`}
            >
        ...
        {finished ? (
          ...
          >
            ...
            <button onClick={restart} className="inline-flex items-center gap-2 px-6 py-3 bg-amber-600 text-white font-semibold rounded-xl hover:bg-amber-700 transition-colors">
          ...
          <div>
            <AnimatePresence mode="wait">
              {!flipped ? (
                ...
                >
                  <p className="text-xs text-stone-400 uppercase tracking-wide mb-4">Hoe zeg je dit in het Spaans?</p>
                  ...
                  <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => setFlipped(true)}
                    className="px-8 py-3 bg-amber-600 text-white font-semibold rounded-xl hover:bg-amber-700 transition-colors flex items-center gap-2"
                  >
                ...
                <motion.div key={`back-${index}`} initial={{ rotateY: -180, opacity: 0 }} animate={{ rotateY: 0, opacity: 1 }} exit={{ rotateY: 90, opacity: 0 }} transition={{ duration: 0.35 }}
                  className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-8 border-2 border-amber-200 shadow-md flex flex-col items-center justify-center min-h-[280px] text-center"
                >
                  <p className="text-xs text-amber-400 uppercase tracking-wide mb-2">Antwoord</p>
                  <p className="text-2xl font-bold text-stone-800 mb-1">{current.spanish}</p>
                  {current.pronunciation && <p className="text-sm text-amber-600 font-mono mb-1">{current.pronunciation}</p>}
                  <p className="text-sm text-stone-500 mb-4">{current.dutch}</p>
                  <button onClick={() => speak(current.spanish)}
                    className={`mb-6 p-3 rounded-full transition-colors ${speaking ? "bg-amber-200 text-amber-700 animate-pulse" : "bg-amber-100 text-amber-600 hover:bg-amber-200"}`}
        ...
        {!finished && cards.length > 0 && (
          <div className="w-full bg-stone-200 rounded-full h-1.5">
            <motion.div className="bg-amber-500 h-1.5 rounded-full" animate={{ width: `${((index) / cards.length) * 100}%` }} transition={{ type: "spring", stiffness: 300, damping: 30 }} />
          </div>
        )}
      </div>
    </Layout>
