...
import { frenchTravelPhrases, FrenchPhraseCategory, FrenchTravelPhrase } from "@/data/phrases/french-travel";

/** Pick the best available French female voice */
function getFrenchVoice(): SpeechSynthesisVoice | null {
  ...
  const preferred = [
    "Google français", "Amélie", "Audrey", "Thomas",
    "Microsoft Julie", "Microsoft Denise", "Marie",
  ];
  for (const name of preferred) {
    const match = frVoices.find((v) => v.name.includes(name));
    if (match) return match;
  }
  const female = frVoices.find(
    (v) =>
  ...
  return female || frVoices[0] || null;
}

function PhraseCard({ phrase, index }: { phrase: FrenchTravelPhrase; index: number }) {
...
function CategorySection({ category }: { category: FrenchPhraseCategory }) {
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        className="w-full flex items-center gap-3 bg-white rounded-2xl p-4 border border-stone-200 shadow-sm hover:shadow-md hover:border-blue-200 transition-all"
      >
...
export default function ReiszinnenFransPage() {
  ...
  const totalPhrases = frenchTravelPhrases.reduce(
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-stone-500 text-sm">
            {totalPhrases} essentiële zinnen voor toeristen in Frankrijk — met uitspraak
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Zoek een zin (NL of Frans)..."
            className="w-full pl-10 pr-4 py-3 rounded-xl border border-stone-200 bg-white text-sm outline-none focus:border-blue-400 transition-colors"
          />
        </div>
